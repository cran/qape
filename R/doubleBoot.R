...
{
if (class(predictor) != "EBLUP" & class(predictor) != 
    "plugInLMM" & class(predictor) != "ebpLMMne") {
...
estQAPE <- sapply(1:number_of_predictors, function(i) 
  quantile(abs(error1[i,]), probs = p))
...
qape17 <- sapply(1:number_of_predictors, function(i) quantile(sqrt(u1_2mod)[, 
                                                                            i], probs = p))
qape18 <- sapply(1:number_of_predictors, function(i) quantile(sqrt(u2_2mod)[, 
                                                                            i], probs = p))
qape19 <- sapply(1:number_of_predictors, function(i) quantile(sqrt(u3_2mod)[, 
                                                                            i], probs = p))
...
return(list(estMSE_param = mse4, estMSE_db_B2 = mse8, estMSE_db_B2_WDZ = mse8mod, 
            ...
            error1 = error1,
            error2 = error2,
            corSquaredError1_db_B2 = u1_2,
            corSquaredError1_db_1 = u2_2,
            corSquaredError1_db_telesc = u3_2,
            corSquaredError1_db_B2_WDZ = u1_2mod,
            corSquaredError1_db_1_WDZ = u2_2mod,
            corSquaredError1_db_telesc_WDZ = u3_2mod))
}